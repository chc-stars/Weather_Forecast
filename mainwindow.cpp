    ...
    , ui(new Ui::MainWindow)
{
    ...
    setFixedSize(QSize(800, 450));   // 设置固定窗口大小
    ...
    // 事件过滤器，用于拦截框架发送的事件QEvent::Point事件（标签调用update函数后会自动发送的）
...
void MainWindow::parseJson(QByteArray data){
    ...
    mDay[0].aqi = yesterdayObj.value("aqi").toDouble();  // 空气质量
    mDay[0].type = yesterdayObj.value("type").toString(); //天气类型
    mDay[0].week = yesterdayObj.value("week").toString(); //星期

    // 解析最低温度和最高温度
    mDay[0].low = parseString(yesterdayObj.value("low").toString());  // 最低温
...
{
    str = str.split(" ").at(1); // 以空格为分隔符，去除前面的文字，取出第二个x℃
...
void MainWindow::UpDateUI()
{
    ...
    // 更新icon
    ui->lbTemp->setPixmap(mTypeMap[mToday.type]);
...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
    if(watched == ui->lblHighCurve && event->type()== QEvent::Paint){
        // 是否是lblHighCurve且事件类型为paint
        paintHighCurve();
    }
    if(watched == ui->lblLowCurve && event->type() == QEvent::Paint){
        // 是否是lblLowCurve且事件类型为paint
        paintLowCurve();
    }
    // 执行完后，继续父类的操作
